...
// 1) Alle Plan-Details zentral definieren
const PLAN_DETAILS = {
  free: {
    key: 'free',
    name: 'Free',
...
export default function CheckoutPage() {
  ...
  const planKeyFromQuery = searchParams.get('plan');

  // 2) Wenn planKey ungültig oder leer, verwende 'free' als Fallback
  const planKey =
    typeof planKeyFromQuery === 'string' && PLAN_DETAILS[planKeyFromQuery]
      ? planKeyFromQuery
      : 'free';
  ...
  // 7) Wenn im Query ein ungültiger planKey steht, leiten wir zurück zu „plan=free“
  useEffect(() => {
    if (!PLAN_DETAILS[planKeyFromQuery]) {
      router.replace('/checkout?plan=free');
    }
  }, [planKeyFromQuery, router]);
    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl w-full grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-800 rounded-2xl p-8 shadow-lg flex flex-col">
          ...
          {/* Credit Card Form (nur wenn „card“ ausgewählt) */}
          {selectedMethod === 'card' && (
