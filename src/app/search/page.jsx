...
const getRandomImage = (topic = "esports") =>
  `https://source.unsplash.com/random/800x450?${topic}&sig=${Math.random()}`;
...
function SearchResultsContent() {
  ...
  const contentTemplates = useMemo(() => ({
    videos: shuffledDemoResults.map((demo) => ({
      ...
      channelAvatar: getRandomImage("avatar"),
      watched: false,
    ...
    utilities: Array.from({ length: 8 }).map((_, i) => ({
      ...
      thumbnail: getRandomImage("utility"),
    ...
    events: Array.from({ length: 5 }).map((_, i) => ({
      ...
      thumbnail: getRandomImage("event"),
      ...
      matches: Array.from({ length: 8 }).map((__, j) => ({
        ...
        thumbnail: getRandomImage("match"),
