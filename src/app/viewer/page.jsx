...
const ScoreDisplay = ({
  ...
  return (
    <div className="bg-white/10 p-4 rounded-lg shadow-lg mb-4 text-white">
      ...
      <div
        className="text-4xl font-bold text-cyan-300 tracking-wider"
        style={{ textShadow: "0 0 8px rgba(56, 189, 248, 0.7)" }}
      >
        <CountUp
        ...
        <Droppable droppableId="inventoryGrid" direction="horizontal">
          {(provided, snapshot) => (
            <div
              ...
              className={`grid grid-cols-4 gap-2 p-2 rounded-md transition-colors duration-300 ${
                snapshot.isDraggingOver ? "bg-cyan-500/20" : "bg-black/20"
              }`}
            >
              {state.slots.map((item, index) => (
                <Draggable
                  key={item ? item.id : `empty-${index}`}
                  draggableId={item ? item.id : `empty-${index}`}
                  index={index}
                  isDragDisabled={!item}
                >
                  {(providedDraggable, snapshotDraggable) => (
                    <div
                      ...
                      className={`h-20 w-full rounded-md border-2 flex items-center justify-center relative ${
                        snapshotDraggable.isDragging
                          ? "border-cyan-400 scale-110 shadow-2xl z-10"
                          : "border-gray-600"
                      } ${
                        item ? "bg-gray-700/80 cursor-grab" : "bg-gray-800/50"
                      }`}
                    >
                      {item ? (
                        <div className="text-center">
                          ...
                          {item.quantity > 1 && (
                            <div className="absolute -top-1 -right-1 bg-cyan-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                              {item.quantity}
                            </div>
                          )}
                        </div>
...
const HeatmapView = ({ dataPoints, width, height }) => {
  ...
  useEffect(() => {
    if (typeof window.h337 === "undefined" || !heatmapContainerRef.current)
      return;
    const heatmapInstance = window.h337.create({
      ...
      minOpacity: 0.1,
      blur: 0.85,
      gradient: { ".1": "blue", ".5": "cyan", ".7": "yellow", ".95": "red" },
    });
    if (dataPoints) {
  ...
  return (
    ...
    >
      <Layer>
        ...
        <Text
          text="Willkommen im 2D Demo Viewer"
          fontSize={24}
          x={20}
          y={20}
          fill="white"
...
const DemoViewerContent = () => {
  ...
  const getMockData = () => ({
    scene: {
      backgroundImageUrl:
        "https://placehold.co/1200x800/2c3e50/ffffff?text=2D+Spielwelt",
      items: [
        {
          id: "item-1",
          x: 150,
    ...
    heatmap: {
      maxValue: 100,
      dataPoints: [
        { x: 180, y: 230, value: 80, radius: 50 },
        { x: 450, y: 370, value: 95, radius: 60 },
        { x: 700, y: 500, value: 40, radius: 70 },
        { x: 200, y: 550, value: 60, radius: 55 },
      ],
    },
    inventory: {
      slots: [
        {
          id: "inv-1",
          name: "Heiltrank",
          icon: "https://placehold.co/64x64/e74c3c/ffffff?text=HP",
          quantity: 3,
        },
        {
          id: "inv-2",
          name: "Schlüssel",
          icon: "https://placehold.co/64x64/f1c40f/ffffff?text=Key",
          quantity: 1,
        },
        null,
        {
          id: "inv-3",
          name: "Münze",
          icon: "https://placehold.co/64x64/2ecc71/ffffff?text=Gold",
          quantity: 42,
        },
        null,
        null,
        ...
        null,
        {
          id: "inv-4",
          name: "Schwert",
          icon: "https://placehold.co/64x64/95a5a6/ffffff?text=Swrd",
          quantity: 1,
        },
        null,
        null,
  ...
  useEffect(() => {
    const scoreInterval = setInterval(() => {
      ...
      setScores((prev) => ({
        ...prev,
        points: prev.points + Math.floor(Math.random() * 500),
      }));
    }, 5000);
    ...
    <div className="bg-gray-800 font-sans text-white h-screen w-screen flex flex-col p-4 gap-4">
      <header className="flex-shrink-0">
        <h1 className="text-2xl font-bold text-center text-cyan-300">
          Umfangreicher 2D Demo Viewer
        </h1>
      </header>
      <div className="flex-grow flex flex-col md:flex-row gap-4 min-h-0">
        <main
          ref={containerRef}
          className="flex-grow bg-gray-900 rounded-xl shadow-2xl relative overflow-hidden"
        >
          {dimensions.width > 0 && (
            <>
              <MainCanvas
                sceneData={mockData.scene}
              ...
              />
              <HeatmapView
                dataPoints={mockData.heatmap.dataPoints}
                width={dimensions.width}
                height={dimensions.height}
              />
            </>
        ...
        <aside className="w-full md:w-96 flex-shrink-0 flex flex-col gap-4 overflow-y-auto pr-2">
          <ScoreDisplay
            score={scores.points}
            previousScore={prevScores.points}
            label="Punktestand"
            suffix=" Pkt."
          />
          <ScoreDisplay
            score={scores.level}
            previousScore={prevScores.level}
            label="Level"
            prefix="Lv. "
          />
      ...
      <div className="bg-gray-800 font-sans text-white h-screen w-screen flex items-center justify-center">
        <div className="text-center p-8 bg-gray-900 rounded-xl shadow-2xl">
          <h1 className="text-2xl font-bold text-cyan-300">
            Lade Viewer-Bibliotheken...
          </h1>
          <p className="text-gray-400 mt-2">
            Die interaktive Demo wird vorbereitet.
          </p>
        </div>
      </div>
    );
