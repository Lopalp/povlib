...
const MapCanvas = ({ players, geometry, showHeatmap, showPaths, events, currentTime, isMultiRound }) => {
    ...
    useEffect(() => {
        ...
        const ctx = canvas.getContext('2d'); const dpr = window.devicePixelRatio || 1; const rect = canvas.getBoundingClientRect(); canvas.width = rect.width * dpr; canvas.height = rect.height * dpr; const scaleX = canvas.width / 1600; const scaleY = canvas.height / 900; ctx.scale(dpr, dpr); ctx.clearRect(0, 0, canvas.width, canvas.height); ctx.fillStyle = theme.colors.mapBackground; ctx.fillRect(0, 0, rect.width, rect.height);
        ctx.save(); ctx.scale(scaleX, scaleY);
        ctx.fillStyle = theme.colors.mapWall; geometry.walls.forEach(wall => ctx.fillRect(wall.x, wall.y, wall.w, wall.h));
        if (showHeatmap) { events.filter(e => e.type === 'kill').forEach(event => { const g = ctx.createRadialGradient(event.position.x, event.position.y, 10, event.position.x, event.position.y, 80); g.addColorStop(0, 'rgba(251, 191, 36, 0.3)'); g.addColorStop(1, 'rgba(251, 191, 36, 0)'); ctx.fillStyle = g; ctx.fillRect(event.position.x - 80, event.position.y - 80, 160, 160); }); }
        players.forEach(p => {
            const playerPos = isMultiRound ? p.path[p.path.length-1] : p.path.find(pos => pos.t >= currentTime) || p.path[0];
            if (showPaths) { ctx.beginPath(); ctx.moveTo(p.path[0].x, p.path[0].y); const pathToShow = isMultiRound ? p.path : p.path.filter(pos => pos.t <= currentTime); pathToShow.forEach(pos => ctx.lineTo(pos.x, pos.y)); ctx.strokeStyle = p.team === 'teamA' ? theme.colors.teamA : theme.colors.teamB; ctx.lineWidth = 2; ctx.globalAlpha = 0.4; ctx.stroke(); ctx.globalAlpha = 1; }
            ctx.beginPath(); ctx.arc(playerPos.x, playerPos.y, 12, 0, 2 * Math.PI); ctx.fillStyle = p.team === 'teamA' ? theme.colors.teamA : theme.colors.teamB; ctx.globalAlpha = isMultiRound || p.isAlive ? 1.0 : 0.3; ctx.fill(); ctx.strokeStyle = '#000'; ctx.lineWidth = 1.5; ctx.stroke();
            if ((isMultiRound && !p.isAlive) || (!isMultiRound && !p.isAlive)) { const skullPos = p.path[p.path.length-1]; ctx.fillStyle = '#FFF'; ctx.font = 'bold 18px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('💀', skullPos.x, skullPos.y); }
        });
        ctx.restore();
                            ...
                            <div className="flex items-center justify-center px-4 pt-2">
                             ...
                             {!isMultiRound && (
                                <div className="flex items-center gap-3 text-white pt-2 border-t border-gray-700/50">
                                     <IconButton onClick={() => setIsPlaying(!isPlaying)}>{isPlaying ? <Pause /> : <Play />}</IconButton>
                                     <span className="text-sm font-mono w-14 text-center">{formatTime(currentTime)}</span>
                                     <input type="range" min="0" max={roundDuration} value={currentTime} onChange={(e) => setCurrentTime(parseFloat(e.target.value))} className="w-full h-2 bg-white/20 rounded-lg appearance-none cursor-pointer accent-color"/>
