  ...
  try {
    ...
    if (playerError) {
      ...
      if (demos && demos.length > 0) {
        ...
        const totalViews = demos.reduce((sum, demo) => sum + (demo.views || 0), 0);
        ...
        playerData = {
          ...
          stats: {
            ...
            avgRating: demos.some(d => d.rating) 
              ? (demos.reduce((sum, d) => sum + (d.rating || 0), 0) / demos.filter(d => d.rating).length).toFixed(2)
              : null
          }
        };
      } else {
...
export async function getAllPlayers(type = 'pro', page = 1, pageSize = 20, filters = {}) {
  try {
      ...
      demos.forEach(demo => {
        ...
        demo.players.forEach(playerName => {
          if (!playerMap[playerName]) {
            ...
            const playerDemos = demos.filter(d => d.players && d.players.includes(playerName));
            const teamCounts = {};
            ...
            const totalViews = playerDemos.reduce((sum, d) => sum + (d.views || 0), 0);
    ...
    playersData.sort((a, b) => (b.stats?.totalViews || 0) - (a.stats?.totalViews || 0));
