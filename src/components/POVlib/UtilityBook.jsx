...
export default function UtilityBook() {
  ...
  const groups = useMemo(() => ['All', ...new Set(dummyUtils.map(u => u.type))], []);
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filtered.map(util => (
          <div key={util.id} className="bg-gray-800 rounded-lg overflow-hidden shadow-sm">
        ...
        {filtered.length === 0 && (
          <p className="text-gray-400 col-span-full">No utils match your filter.</p>
        )}
      </div>
    </section>
  );
