      ...
      <div className="bg-gray-900 pt-16">
        <div className="container mx-auto px-4 pb-12">
          <div className="flex items-center gap-4 mb-6 flex-wrap">
            <button onClick={onClose} className="flex items-center text-gray-400 hover:text-yellow-400 mt-2">
              <ArrowLeft className="h-5 w-5 mr-2" /> Back to Browse
            </button>
            <Link href="/videoplaypagecopy">
              <a className="text-gray-400 hover:text-yellow-400 mt-2">Go to Copy Page</a>
            </Link>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="w-full lg:w-8/12">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-4">
                ...
                {/* Actions including Additional Features */}
                <div className="flex flex-wrap items-center space-x-4 text-gray-400">
                  <button onClick={() => onLike(selectedDemo.id)} className="flex items-center mb-2">
                    <ThumbsUp className="h-5 w-5" /> <span className="ml-1">{selectedDemo.likes}</span>
                  </button>
                  <button onClick={onOpenTagModal} className="flex items-center mb-2">
                  ...
                  <button className="flex items-center hover:text-red-500 mb-2">
                    <Flag className="h-5 w-5" /> <span className="ml-1">Report</span>
                  </button>
                  {/* Additional Features Icons */}
                  <a href={selectedDemo.video_url} download className="flex items-center hover:text-yellow-400 mb-2">
                    <Download className="h-5 w-5" /> <span className="ml-1 text-sm">Video</span>
                  </a>
                  <a href={selectedDemo.dem_url} download className="flex items-center hover:text-yellow-400 mb-2">
                    <FileText className="h-5 w-5" /> <span className="ml-1 text-sm">Demo</span>
                  </a>
                  <a href={selectedDemo.matchroom_url} target="_blank" rel="noopener noreferrer" className="flex items-center hover:text-yellow-400 mb-2">
                    <ExternalLink className="h-5 w-5" /> <span className="ml-1 text-sm">Matchroom</span>
                  </a>
                </div>
              </div>
                ...
                {selectedDemo.tags?.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 mb-4">
                    {selectedDemo.tags.map(tag => (
                      <span
                        key={tag}
                        className="flex items-center px-3 py-1 bg-gray-700 rounded-full text-sm text-gray-200 hover:bg-yellow-400/20 transition-colors mb-2"
                      >
              ...
              {/* Featured Players */}
              <div className="bg-gradient-to-br from-gray-800 to-gray-850 rounded-lg p-4 border border-gray-700 mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                  {selectedDemo.players.map((player, idx) => (
                    <Link key={idx} href={`/players/${player.replace(/\s+/g, '-').toLowerCase()}`}>
                      <a className="flex items-center gap-2 p-2 rounded-lg hover:bg-gray-700/50 transition-colors">
              ...
              <div className="bg-gradient-to-br from-gray-800 to-gray-850 rounded-lg p-4 border border-gray-700">
                ...
                <div className="space-y-4">
                  {relatedDemos.length ? relatedDemos.map(d => (
                    <div key={d.id} className="flex gap-3 cursor-pointer hover:bg-gray-800/50 p-2 rounded-lg transition-colors flex-wrap">
                      <div className="w-full sm:w-40 h-24 relative overflow-hidden rounded-lg group flex-shrink-0">
