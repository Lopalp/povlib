...
const DemoCardVertical = ({ demo, onSelect }) => {
  return (
    <article
      className="relative w-full max-w-xs mx-auto rounded-2xl overflow-hidden bg-gray-900 shadow-xl cursor-pointer transition-transform hover:scale-105"
      onClick={() => onSelect(demo)}
    >
      <figure className="relative w-full pb-[133%] bg-black">
        <video
          src={demo.videoUrl}
      ...
      <div className="absolute inset-0 flex flex-col justify-end p-4 bg-white/10 backdrop-blur-md">
        <h3 className="text-white text-lg font-bold line-clamp-2">{demo.title}</h3>
        <p className="text-gray-200 text-sm mt-1">{demo.map} · {demo.year}</p>
