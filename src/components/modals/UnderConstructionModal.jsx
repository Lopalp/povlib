  ...
  return (
    ...
    >
      <div className="relative w-full max-w-5xl h-[80vh] md:h-[75vh] rounded-3xl overflow-hidden shadow-2xl transform transition-all duration-500">
        ...
        <div
          className={`absolute inset-0 bg-gradient-to-br ${currentTab.bgGradient} opacity-20 transition-all duration-500`}
        />
        ...
        <div className="relative h-full flex flex-col bg-white/5 dark:bg-gray-900/5 backdrop-blur-md border border-white/30 dark:border-white/20">
            ...
            <div className="flex justify-center items-center gap-3 mb-6">
              {tabs.map((tab, idx) => (
                ...
                >
                  ...
                  {idx === currentIndex && (
                    <div
                      className={`absolute inset-0 rounded-full bg-gradient-to-r ${tab.gradient} blur-md opacity-50`}
                    />
                  )}
                </button>
              ))}
          ...
          {/* Scrollable Content */}
          <div
            className={`flex-1 px-6 md:px-8 pb-6 overflow-y-auto transition-all duration-300 ${
          ...
          >
            ...
            <div className="flex justify-center mb-8">
              <div className="relative group">
                <div
                  className={`absolute inset-0 rounded-3xl bg-gradient-to-br ${currentTab.gradient} blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-300`}
                />
                <div
                  className={`relative p-7 rounded-3xl bg-gradient-to-br ${currentTab.gradient} bg-opacity-70 shadow-2xl transform transition-all duration-300 group-hover:scale-110 backdrop-blur-sm`}
                >
                  <div className="text-white">{currentTab.icon}</div>
                </div>
              </div>
            </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      <div
                        className={`w-8 h-8 rounded-full bg-gradient-to-r ${currentTab.gradient} opacity-20`}
                      />
                      <div
                        className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-gradient-to-r ${currentTab.gradient}`}
                      />
                    </div>
                    <span className="text-gray-700 dark:text-gray-100 font-medium text-sm md:text-base">
                      {feature}
                    </span>
                  </div>
                  <div
                    className={`absolute inset-0 bg-gradient-to-r ${currentTab.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-300`}
                  />
                </div>
              ))}
            </div>
