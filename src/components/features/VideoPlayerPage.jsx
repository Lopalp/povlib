...
import YouTubeEmbed from "../media/YouTubeEmbed"; // Ihre NEUE, saubere Komponente
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-200 pt-24">
      <main className="pb-0">
        ...
        >
          {/* Dieser Container behält das 16:9 Seitenverhältnis bei */}
          <div className="aspect-video">
            <YouTubeEmbed
              ...
              autoplay
              controls={false} // Oft besser, die YouTube-Controls bei Overlays auszublenden
              showInfo={false}
              // WICHTIG: Diese Klassen positionieren das Video absolut, um den Container auszufüllen
              className="absolute top-0 left-0 w-full h-full"
            />
          </div>

          {/* Overlays (WASD, Timeline, Toolbar) bleiben unverändert */}
          {showKeyOverlay && (
                ...
                {selectedRound && (
                  <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700 animate-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-gray-900/50 p-3 rounded-lg"><div className="text-xs text-gray-400 mb-1">Round Type</div><div className="text-white font-medium">{selectedRound <= 15 ? 'First Half' : 'Second Half'}</div></div>
                      <div className="bg-gray-900/50 p-3 rounded-lg"><div className="text-xs text-gray-400 mb-1">Economy</div><div className="text-yellow-400 font-medium">{Math.random() > 0.5 ? 'Force Buy' : 'Full Buy'}</div></div>
                      <div className="bg-gray-900/50 p-3 rounded-lg"><div className="text-xs text-gray-400 mb-1">Duration</div><div className="text-white font-medium">{Math.floor(Math.random() * 60 + 30)}s</div></div>
                    </div>
          ...
          <div
            className={`absolute bottom-4 right-4 flex items-center gap-3 bg-gray-900/70 backdrop-blur-sm border border-gray-700 rounded-lg p-2 transition-opacity duration-300 pointer-events-auto z-10`}
          >
